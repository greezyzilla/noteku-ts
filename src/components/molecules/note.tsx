...
export default function Note(props : NoteProps) {
  ...
  const buttonClass = 'flex-1 text-sm justify-center flex p-5 hover:scale-x-110 hover:scale-y-110 hover:text-white transition';
  const buttonNormalClass = `${buttonClass} bg-slate-100 text-slate-500 hover:bg-slate-500 hover:text-white `;
  const buttonActiveClass = `${buttonClass} bg-slate-500 text-white hover:bg-slate-600 hover:text-white `;
  ...
  return (
    <div key={id} className="flex flex-col gap-2 box-border border border-slate-300 hover:ring-4 ring-slate-100 rounded-md min-h-[350px]">
